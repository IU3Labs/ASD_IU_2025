...
public class TopFreq {
    ...
    public static void main(String[] args) {
        ...
        int n = Util.input(); // считываем размер массива
...
/**
 * 1. Проходим по массиву один раз = O(n)
 * 2. Перенос в список List<int[]>: Количество уникальных чисел = O(m), m <= n, тогда в худшем случае = O(n)
 * 3. Сортировка списка по частоте, m элементов → O(m log m), m <= n, тогда в худшем случае = O(n log n)
 * 4. Выбор k наиболее частых элементов: Проходим по k элементам → O(k), k <= n, тогда в худшем случае = O(n)
 * 5. Сложность алгоритма: n+n+n*log(n)+n = 3n+n*log(n) = O(n log n)
 * */